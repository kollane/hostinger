...
Selles laboris deploy'ad Lab 2's loodud **kõik kolm mikroteenust** (Todo Service, User Service, Frontend) Kubernetes cluster'isse.

Õpid Kubernetes põhikontseptsioone: Pods, Deployments, Services, ConfigMaps, Secrets ja Persistent Volumes täisfunktsionaalse rakenduse näitel.
...
**Täielik mikroteenuste süsteem Kubernetes'es:**
...
│  │  ┌─────────────┐        ┌─────────────┐             │ │
│  │  │     PVC     │        │     PVC     │             │ │
│  │  │  users-data │        │  todos-data │             │ │
│  │  └─────────────┘        └─────────────┘             │ │
│  │                                                      │ │
│  │  ConfigMaps: user-config, todo-config, frontend-cfg │ │
│  │  Secrets: db-users-creds, db-todos-creds, jwt-secret│ │
│  │                                                      │ │
│  └──────────────────────────────────────────────────────┘ │
└────────────────────────────────────────────────────────────┘
```

**Teenused Kubernetes'es:**
- **Frontend**: 1 replica (NodePort :30080)
- **User Service**: 2 replicas (ClusterIP :3000) + PostgreSQL (users DB)
- **Todo Service**: 2 replicas (ClusterIP :8081) + PostgreSQL (todos DB)
...
- [x] **Labor 1: Docker Põhitõed** - KOHUSTUSLIK
  - Vaja on Lab 1'st loodud Docker image:
    - `todo-service:1.0` (Java Spring Boot backend - LAB 1 PÕHIFOOKUS)
  - Dockerfile'ide ja konteinerite mõistmine

- [x] **Labor 2: Docker Compose** - KOHUSTUSLIK
  - Vaja on Lab 2's loodud kõiki kolme image'i:
    - `todo-service:1.0` (Lab 1'st)
    - `user-service:1.0` (Lab 2's loodud)
    - `frontend:1.0` (Lab 2's loodud)
  - Multi-container rakenduste kogemus
  - Mõistmine, kuidas teenused omavahel suhtlevad
